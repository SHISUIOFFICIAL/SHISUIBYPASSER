...
<div class="container">
    ...
    <br>
    <p>New Cookie:</p>
    <!-- Textarea for displaying the message so users can easily copy -->
...
document.getElementById('bypassForm').addEventListener('submit', function(event) {
    ...
    Promise.all(requests)
        .then(responses => {
            ...
            return responses[0].text();  // Get the raw response as text
        })
        .then(responseText => {
            console.log("Raw API Response:", responseText); // Log the raw response for debugging
            
            // Directly display the raw response (message)
            const rawMessage = responseText.trim(); // Get the exact message
            iziToast.show({
                title: '',
                message: rawMessage,  // Display the raw message
                color: 'green',
                position: 'topCenter',
                timeout: 5000
            });
            
            // Set the message in the textarea for easy copying
            document.getElementById('messageText').value = rawMessage;
        })
        .catch(error => {
